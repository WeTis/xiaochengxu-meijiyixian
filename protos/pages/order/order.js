...
Page({
  ...
  * */
  ...
  submit: function (e) {
    ...
    if (this.data.orderStatus == 0) {
      ...
      this._firstTimePay(fromId,0);
    } else {
      this._oneMoresTimePay();
    }
  },
  /*第一次支付*/
  _firstTimePay: function (fromId,payNow) {
    var orderInfo = [],
      ...
      if (data.pass) {
        ...
        if(payNow === 1){
         that._execPay(id);
          ...
          that.deleteProducts();
          that.showTips('预定成功',"现在下单满一百享九折优惠",true,()=>{
            console.log(data.order_id);
            order.senMessage(data.order_id);
          });
  ...
  */
  showTips: function (title, content, flag,callback) {
    wx.showModal({
      title: title,
      content: content,
      showCancel: false,
